...
WHERE Trainer_ID IN (SELECT Trainer_ID
   ...
   while(($row = pg_fetch_row($result)) && $i<16)
   {
      ...
      echo '<div style="font-family: georgia;font-size: 14px;font-weight: bold;color:#F9D342;"><a href="TrainerInfo.php?id='.$row[0].'" style="text-decoration: none;color:#F9D342;">';
    echo '<div class= "column" >'.'<br />'.'<img src = "./Trainer/'.strtolower($row[1]).'.png" height = "300" width = "200" style="border-radius:10%;" >'.'<br /><center>'.strtoupper($row[0]).' <br/>    '.$row[1].'</center></div>'."\n";
echo '</a></div>';
    $conuter=$conuter+1;
    if($conuter%4==0){
      echo '</div>';
