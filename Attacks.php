...
<table>
  ...
  <tr><?php 
  $query = "SELECT Max(PP),Max(Accuracy),Max(Power),Min(Power),Min(Accuracy),Min(PP),round(cast(Avg(PP) as numeric),2),round(cast(Avg(Accuracy) as numeric),2),round(cast(Avg(Power) as numeric),2) FROM Attacks;";
      $result = pg_query($db_connection, $query);
      if(!$result)
      {
        echo pg_last_error($db_connection);
...
<div style="top:30px;left:400px;position: relative;width:1050px;">
