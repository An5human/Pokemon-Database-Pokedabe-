...
<div style="top:0px;left:420px;height:100%;width:1050px;position: relative;">
<?php
   $query = "SELECT Badge_name,region,name FROM Pokemon_gym join Trainer on gym_leader = Trainer_id;";
   ...
   while(($row = pg_fetch_row($result)))
   {
      ...
      echo '<div style="font-family: georgia;font-size: 14px;font-weight: bold;color:#F9D342;"><a href="PokeBadge.php?name='.$row[0].'" style="text-decoration: none;color:#F9D342;">';
    echo '<div class= "column" >'.'<br />'.'<img src = "./Badges/'.$row[0].'.png" height = "300" width = "200" style="border-radius:2%;" >'.'<br /><center>'.strtoupper($row[0]).' <br/>      '.$row[1].' <br/>     '.$row[2].'</center></div>'."\n";
echo '</a></div>';
    $conuter=$conuter+1;
    if($conuter%4==0){
      echo '</div>';
