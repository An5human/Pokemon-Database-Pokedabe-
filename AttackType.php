...
<a href="/Attacks.php" style="color:#292826;text-decoration: none"><div style="background-color: #292826;font-size:35px;color:#F9D342;">Attacks</div></a>
...
<center><H1>ATTACKS OF <?php echo $Type;?> IS</H1>
...
<?php 
$query = "SELECT Count(*) FROM Attacks WHERE type = '".$Type."';";
...
<table>
  ...
  <tr><?php 
  $query = "SELECT Max(PP),Max(Accuracy),Max(Power),Min(Power),Min(Accuracy),Min(PP),round(cast(Avg(PP) as numeric),2),round(cast(Avg(Accuracy) as numeric),2),round(cast(Avg(Power) as numeric),2) FROM Attacks where type='".$Type."';";
      $result = pg_query($db_connection, $query);
      if(!$result)
      {
        echo pg_last_error($db_connection);
...
<center>
<?php
  $query = "SELECT * FROM Attacks WHERE type = '".$Type."';";
   $result = pg_query($db_connection, $query);
   if(!$result)
   {
      echo pg_last_error($db_connection);
