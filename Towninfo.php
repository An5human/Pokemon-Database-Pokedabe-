...
<center><H1>Pokemon Found In <?php echo $name;?></H1>
   <?php
  	$query = "SELECT COUNT(*) FROM pokemon_found_in_town WHERE town_name ='".$name."';";
...
<center><b><h1>Pokemon Found Here:</h1></b></center>
    <br />
<?php
  $query = "SELECT name,type1,type2,description FROM pokemon join pokemon_found_in_town on pokemon_name = name where town_name = '".$name."';";
   $result = pg_query($db_connection, $query);
   if(!$result)
   {
      echo pg_last_error($db_connection);
